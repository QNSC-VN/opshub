...
describe('resolveDatabaseUrl', () => {
  ...
  it('composes from parts when no URL is supplied', () => {
    expect(resolveDatabaseUrl(parts)).toBe(
      'postgresql://app_admin:plainpw@db.internal:5432/opshub?sslmode=require',
    );
  });

  it('accepts a numeric port', () => {
    // The env schema coerces DATABASE_PORT to a number, so the deployed caller passes
    // one even though Terraform injected a string.
    expect(resolveDatabaseUrl({ ...parts, DATABASE_PORT: 5432 })).toContain(':5432/opshub');
  });

  it('defaults sslmode to require', () => {
    expect(resolveDatabaseUrl(parts)).toMatch(/sslmode=require$/);
  });
  ...
  // AWS-generated passwords routinely contain URL-structural characters. Left raw these
  // either fail to parse or — worse — parse into a DIFFERENT host, which is a silent
  ...
  it('round-trips the encoded password back to the original', () => {
    const password = 'p@:/?#ss';
    const url = new URL(resolveDatabaseUrl({ ...parts, DATABASE_PASSWORD: password }));
