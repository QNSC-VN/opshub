...
export interface IAssetRepository {
  create(input: CreateAssetInput, tx?: DbExecutor): Promise<Asset>;
  ...
  /** Mark asset retired. */
  retire(assetId: string, tx?: DbExecutor): Promise<void>;

  listAssignments(assetId: string): Promise<AssetAssignment[]>;

  /** Update the S3 object key for the asset photo. Pass null to clear. */
  updatePhoto(assetId: string, photoStorageKey: string | null, tx?: DbExecutor): Promise<void>;
}
