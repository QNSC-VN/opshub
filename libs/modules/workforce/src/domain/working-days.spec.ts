...
import { datesInRange, isoWeekday, WORKING_WEEKDAYS, workingDatesInRange } from './working-days';
...
describe('workingDatesInRange', () => {
  it('counts a single working day as one, not zero', () => {
    ...
    expect(workingDatesInRange('2026-03-04', '2026-03-04').length).toBe(1);
  });

  it('counts a full Mon–Fri week as five', () => {
    expect(workingDatesInRange('2026-03-02', '2026-03-06').length).toBe(5);
  });

  it('excludes the weekend inside a span', () => {
    // Mon 2nd → Mon 9th is eight calendar days, six of them working.
    expect(workingDatesInRange('2026-03-02', '2026-03-09').length).toBe(6);
  });

  it('returns 0 for a weekend-only span', () => {
    // Sat + Sun. A real answer, not an error — the caller decides what to do with a
    // zero-cost request.
    expect(workingDatesInRange('2026-03-07', '2026-03-08').length).toBe(0);
  });

  it('excludes a holiday that falls on a working day', () => {
    expect(workingDatesInRange('2026-03-02', '2026-03-06', new Set(['2026-03-04'])).length).toBe(4);
  });
  ...
  it('does not double-count a holiday that falls on a weekend', () => {
    ...
    expect(workingDatesInRange('2026-03-02', '2026-03-09', new Set(['2026-03-07'])).length).toBe(6);
  });

  it('ignores holidays outside the window', () => {
    expect(workingDatesInRange('2026-03-02', '2026-03-06', new Set(['2026-04-01'])).length).toBe(5);
  });
...
describe('isoWeekday', () => {
  // Pinned SEPARATELY from workingDatesInRange because it cannot be observed through it: Sunday as
