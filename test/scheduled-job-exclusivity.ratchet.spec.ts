 ...
 *
 * Why this exists: eight scheduled jobs had no cross-pod guard at all. `@Cron` and
 * `@Interval` fire on EVERY replica, so a rolling deploy that briefly overlaps two worker
 * tasks ran all eight twice, concurrently — including `audit-cleanup`, which DELETES audit
 * rows, and `storage-cleanup`, which deletes objects. Three of them carried a private
 * `running` boolean that reads like protection but only ever guarded one process.
 *
 * Nothing could have caught this by testing: the worker runs at `min_count = 0` with
 * autoscaling off, so a single replica made every job look correctly serialised.
 *
